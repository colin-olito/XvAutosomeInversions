...
wmbar  <-  function(xi, Wm.vec, ...) { 
	xi[1]*Wm.vec[1] + xi[2]*Wm.vec[2] + xi[3]*Wm.vec[3] + xi[4]*Wm.vec[4] + xi[5]*Wm.vec[5]
}
...
yPr.1  <-  function(xi = xi, yi = yi, Wm.vec = Wm.vec, par.list = par.list, ...) {
	(xi[1]*Wm.vec[1] / wmbar(xi=xi, Wm.vec=Wm.vec))*(1 - par.list$mm) + par.list$mm
} 
yPr.2  <-  function(xi = xi, yi = yi, Wm.vec = Wm.vec, par.list = par.list, ...) {
	(xi[2]*Wm.vec[2] / wmbar(xi=xi, Wm.vec=Wm.vec))*(1 - par.list$mm)
}
yPr.3  <-  function(xi = xi, yi = yi, Wm.vec = Wm.vec, par.list = par.list, ...) {
	(xi[3]*Wm.vec[3] / wmbar(xi=xi, Wm.vec=Wm.vec))*(1 - par.list$mm)
}
yPr.4  <-  function(xi = xi, yi = yi, Wm.vec = Wm.vec, par.list = par.list, ...) {
	(xi[4]*Wm.vec[4] / wmbar(xi=xi, Wm.vec=Wm.vec))*(1 - par.list$mm)
}
yPr.5  <-  function(xi = xi, yi = yi, Wm.vec = Wm.vec, par.list = par.list, ...) {
	(xi[5]*Wm.vec[5] / wmbar(xi=xi, Wm.vec=Wm.vec))*(1 - par.list$mm)
}
...
recursionFwdSim  <-  function(par.list, xi.init, yi.init, threshold = 1e-6, silent = FALSE, ...) {
	...
	##  Preemptive Warnings
	if(any(par.list[1:7] < 0) | any(par.list[2:7] > 1) | par.list$r > 0.5)
	...
	Wf.mat   <-  matrix(
					   c( 1,                             (1 + par.list$h*par.list$sf),                   (1 + par.list$h*par.list$sf),                   (1 + par.list$h*par.list$sf)^2,                 (1 + par.list$h*par.list$sf)^2,
                         (1 + par.list$h*par.list$sf),   (1 + par.list$sf),                              (1 + par.list$h*par.list$sf)^2,                 (1 + par.list$h*par.list$sf)*(1 + par.list$sf), (1 + par.list$h*par.list$sf)*(1 + par.list$sf),
                         (1 + par.list$h*par.list$sf),   (1 + par.list$h*par.list$sf)^2,                 (1 + par.list$sf),                              (1 + par.list$sf)*(1 + par.list$h*par.list$sf), (1 + par.list$sf)*(1 + par.list$h*par.list$sf),
                         (1 + par.list$h*par.list$sf)^2, (1 + par.list$h*par.list$sf)*(1 + par.list$sf), (1 + par.list$sf)*(1 + par.list$h*par.list$sf), (1 + par.list$sf)^2,                            (1 + par.list$sf)^2,
                         (1 + par.list$h*par.list$sf)^2, (1 + par.list$h*par.list$sf)*(1 + par.list$sf), (1 + par.list$sf)*(1 + par.list$h*par.list$sf), (1 + par.list$sf)^2,                            (1 + par.list$sf)^2),
						 nrow=5, byrow=TRUE
					    )
	Wm.vec   <-  c(1, 1 + par.list$sm, 1 + par.list$sm, (1 + par.list$sm)^2, (1 + par.list$sm)^2)
	...
	for (j in 1:ncol(xi.gen)) {
		...
		yi.gen[1, j]     <-  round(recFctY(xi = xi.init, yi = yi.init, par.list = par.list, Wm.vec = Wm.vec), digits=8)
	}
	...
	while (i < par.list$gen & any(diffs[diffs != 0] > threshold)) {
		for (j in 1:ncol(xi.gen)) {
			...
			yi.gen[i, j]   <-  round(recFctY(xi = xi.gen[i-1, ], yi = yi.gen[i-1, ], par.list = par.list, Wm.vec = Wm.vec), digits=8)
		}
	...
	res  <-  list(
				  ...
				  "xi.gen"    =  xi.gen[1:i-1,],
				  "yi.gen"    =  yi.gen[1:i-1,],
				  "Fi.gen"    =  (2*xi.gen[1:i-1,] + yi.gen[1:i-1,])/3,
				  "EQxy.freq" =  c(xi.gen[i-1,], yi.gen[i-1,]),
				  "EQ.freq"   =  (2*xi.gen[i-1,] + yi.gen[i-1,])/3,
				  "LDx"       =  LDx,
				  "LDy"       =  LDy,
				  "LD"        =  (2*LDx + LDy)/3
 				 )
...
#' @param h.vals 	 Values of dominance coefficient to explore (as in par.list). 
...
#' @author Colin Olito.
#' @examples
#' recursionFwdSimLoop(gen = 25000, r = 0.5, threshold = 1e-7,
#'					   sf.vals = c(0.01, 0.05), sm.vals = c(0.01, 0.05),
#'					   mf.vals = c(0.01, 0.05), mm.vals = c(0.01, 0.05),
#'					   h.vals = c(0.0, 0.01, 0.1))

recursionFwdSimLoop  <-  function(gen = 25000, r = 0.5, threshold = 1e-7,
								  sf.vals = c(0.01, 0.05), sm.vals = c(0.01, 0.05),
								  mf.vals = c(0.01, 0.05), mm.vals = c(0.01, 0.05),
								  h.vals = c(0, 0.5, 1)) {
	...
	if(any(c(gen,r,sf.vals,sm.vals,mf.vals,mf.vals,h.vals) < 0) | any(h.vals > 1) | r > 0.5 | any(c(sf.vals, sm.vals,mf.vals,mm.vals) > 0.2))
		stop('At least one of the chosen parameter values fall outside of the reasonable bounds')

	try({if(threshold > 1e-6)
	...
	eqxyFreqs  <-  matrix(0, nrow=length(h.vals)*length(mf.vals)*length(sf.vals), ncol=10)
	LDx        <-  rep(0, length(h.vals)*length(mf.vals)*length(sf.vals))
	LDy        <-  rep(0, length(h.vals)*length(mf.vals)*length(sf.vals))
	
	##  Simulation Loop over values of r, m, s 
	print('Running Deterministic Recursion Simulations')
	pb   <-  txtProgressBar(min=0, max=length(h.vals)*length(mf.vals), style=3)
	...
	setTxtProgressBar(pb, length(h.vals)*length(mf.vals))
	...
	mms  <-  c()
	for(i in 1:length(h.vals)) {
		hs  <-  c(hs, rep(h.vals[i], length(mf.vals)*length(sf.vals)))
	}
	for(i in 1:length(h.vals)) {
		for(j in 1:length(mf.vals)) {
	...
	results.df  <-  data.frame("r"   =  rep(r, length(h.vals)*length(mf.vals)*length(sf.vals)),
							   "sf"  =  rep(sf.vals, length(h.vals)*length(mf.vals)),
							   "sm"  =  rep(sm.vals, length(h.vals)*length(mf.vals)),
							   "h"   =  hs,
							   "mf"  =  mfs,
							   "mm"  =  mms
							   )
	results.df  <-  cbind(results.df, eqxyFreqs, LDx, LDy)
	colnames(results.df)  <-  c('r','sf','sm','h','mf','mm', 
								'x1', 'x2', 'x3', 'x4', 'x5', 
								'y1', 'y2', 'y3', 'y4', 'y5',
								'LDx','LDy')
	
	#  Write results.df to .txt file
	filename  <-  paste("./output/data/simResults/determRecSim-X-linked", "_r", r, ".csv", sep="")
